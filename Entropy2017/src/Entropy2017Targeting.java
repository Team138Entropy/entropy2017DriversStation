...
public class Entropy2017Targeting extends EntropyCameraExtension {
	// Widget Name
	...
	private double[] m_imgResults = {0.0, 1000.0,1000.0,1000.0,1000.0,1000.0};
	...
	public Entropy2017Targeting() throws FileNotFoundException {
	...
	protected void processImage(Mat m) {
		...
		int width = m.cols();
		double angle = (double)((pegx-width/2))/width * 60.0;

		// Send the targeting info to the robot
		m_imgResults[0] = m_frameNumber;
		m_imgResults[1] = angle;
		m_imgResults[2] = pegxspace;
		m_imgResults[3] = pegyheight;
		m_imgResults[4] = highGoalx;
		m_imgResults[5] = highGoalxRange;	
		...
		for (int k=0;k<m_imgResults.length;k++) {
			System.out.println("m_imgResults["+k+"]= " + m_imgResults[k]);
		}
	...
	private void findPeg(Mat m){
	    ...
	    printPeaks("ypeaks",ypeaks);
	    
	    List<PeakLoc> xpeaks = findPeaks(xsums);    
	    printPeaks("xpeaks",xpeaks);
